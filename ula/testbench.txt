...
component divisor8bits is
    port(
        dividend, divisor : in bit_vector(7 downto 0);
        quotient : out bit_vector(7 downto 0);
...
signal a_mult, b_mult, resultado_mult : bit_vector(7 downto 0);
signal overflow_mult : bit;

-- Sinais para o divisor
signal dividend_signal, divisor_signal, quotient_signal, remainder_signal : bit_vector(7 downto 0);
signal div_by_zero_signal : bit;
...
begin
  ...
  -- Instancia do multiplicador (agora com bit shifting)
  DUT_MULTIPLICADOR: multiplicador8bits port map(
    a => a_mult,
    b => b_mult,
  ...
  begin
    ...
    -- Teste 2: Subtracao simples (5 - 3 = 2)
    c_signal <= "00000101";  -- 5
    d_signal <= "00000011";  -- 3
    wait for 1 ns;
    assert(resultado_sub_signal = "00000010") report "Fail 5-3" severity error;
    
    -- Teste 3: Multiplicacao com bit shifting (4 x 4 = 16)
    a_mult <= "00000100";  -- 4
    b_mult <= "00000100";  -- 4
    wait for 1 ns;
    assert(resultado_mult = "00010000") report "Fail 4x4" severity error;
    assert(overflow_mult = '0') report "Fail overflow 4x4" severity error;

    -- =========================================================================
    -- TESTES DO DIVISOR
    -- =========================================================================
    
    -- Teste 4: Divisao simples (8 / 3 = 2, resto 0)
    dividend_signal <= "00001000";  -- 8
    divisor_signal <= "00000011";   -- 3
    wait for 1 ns;
    assert(quotient_signal = "00000010") report "Fail 8/2 quotient" severity error;
    
    --SINAL DO MODULO
    assert(remainder_signal = "00000010") report "Fail 8/2 remainder" severity error;
    assert(div_by_zero_signal = '0') report "Fail 8/2 div_by_zero" severity error;
